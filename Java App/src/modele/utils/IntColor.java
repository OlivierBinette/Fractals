...
/**
 * The Color class defines methods for creating and converting color ints. Colors are represented as
 * packed ints, made up of 4 bytes: alpha, red, green, blue. The values are unpremultiplied, meaning
 * any transparency is stored solely in the alpha component, and not in the color components. The
 * components are stored as follows (alpha << 24) | (red << 16) | (green << 8) | blue. Each
 * component ranges between 0..255 with 0 meaning no contribution for that component, and 255
 * meaning 100% contribution. Thus opaque-black would be 0xFF000000 (100% opaque but no
...
public class IntColor 
{
	...
	/**
	 * Return a color-int from red, green, blue components. The alpha component is implicity 255
	 * (fully opaque). These component values should be [0..255], but there is no range check
	 ...
	 * [0..255], but there is no range check performed, so if they are out of range, the returned
	 * color is undefined.
	 * 
	 ...
	 * #AARRGGBB 'red', 'blue', 'green', 'black', 'white', 'gray', 'cyan', 'magenta', 'yellow',
	 * 'lightgray', 'darkgray'
	 */
	...
	/**
	 * Convert HSB components to an ARGB color. Alpha set to 0xFF. hsv[0] is Hue [0 .. 1) hsv[1] is
	 * Saturation [0...1] hsv[2] is Value [0...1] If hsv values are out of range, they are pinned.
	 * 
	...
	/**
	 * Convert RGB components to HSV. hsv[0] is Hue [0 .. 360) hsv[1] is Saturation [0...1] hsv[2]
	 * is Value [0...1]
	 * 
	...
	/**
	 * Convert the argb color to its HSV components. hsv[0] is Hue [0 .. 360) hsv[1] is Saturation
	...
	/**
	 * Convert HSV components to an ARGB color. Alpha set to 0xFF. hsv[0] is Hue [0 .. 360) hsv[1]
	 * is Saturation [0...1] hsv[2] is Value [0...1] If hsv values are out of range, they are
	 * pinned.
	 * 
	 ...
	 * hsv[0] is Hue [0 .. 360) hsv[1] is Saturation [0...1] hsv[2] is Value [0...1] If hsv values
